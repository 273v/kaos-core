...
class VFSListTool(KaosTool):
    ...
    async def execute(
        ...
        try:
            page = await context.runtime.vfs.list_page(path, limit=limit)
...
class VFSReadTool(KaosTool):
    ...
    async def execute(
        ...
        try:
            data = await context.runtime.vfs.read_range(path, start=start, length=effective_length)
...
class VFSStatTool(KaosTool):
    ...
    async def execute(
        ...
        try:
            stat = await context.runtime.vfs.stat(path)
        except Exception as exc:
            return ToolResult.create_error(
...
class ArtifactsListTool(KaosTool):
    ...
    def metadata(self) -> ToolMetadata:
        return ToolMetadata(
            ...
            description=(
                "List artifacts stored by KAOS tools. Filter by session or workflow. "
                "Use kaos-core-artifacts-inspect to get full details on a specific artifact."
            ),
            category=ToolCategory.DATA,
            capability=ToolCapability.QUERY,
            module_name=_MODULE,

...
class ArtifactStore:
    ...
    async def _resolve_async(
        ...
        *,
        ...
        """Resolve an artifact identifier to its manifest.

        When ``caller_session_id`` is provided, the resolved manifest's
        ``session_id`` must match — otherwise we raise ``ResourceError``
        with the uniform ``"Unknown artifact"`` message so that
        cross-session probing cannot distinguish "not found" from
        "found but forbidden". Pass ``None`` only from trusted
        in-process callers (cleanup, persistence reload).
        """
        if uri_or_id in self._artifacts:
            manifest: ArtifactManifest | None = self._artifacts[uri_or_id]
        elif uri_or_id in self._uri_index:
            manifest = self._artifacts[self._uri_index[uri_or_id]]
        else:
        ...
        if caller_session_id is not None and manifest.session_id != caller_session_id:
            # Uniform "not found" error — never leak cross-session
            # existence information to an unauthorised caller.
    ...
    def get(
        ...
        *,
        ...
        try:
            manifest = self._artifacts[artifact_id]
        except KeyError as exc:
            raise ResourceError("Unknown artifact", artifact_id=artifact_id) from exc
        if caller_session_id is not None and manifest.session_id != caller_session_id:
            # Uniform error — never reveal cross-session existence.
            raise ResourceError("Unknown artifact", artifact_id=artifact_id)
        return manifest
    ...
    def resolve(
        ...
        *,
        ...
        if uri_or_id in self._artifacts:
            manifest = self._artifacts[uri_or_id]
        else:
            try:
    ...
    async def read_body(
        ...
        *,
        ...
        manifest = await self._resolve_async(artifact_id, caller_session_id=caller_session_id)
    ...
    async def read_text(
        ...
        *,
        ...
        return (
            await self.read_body(artifact_id, max_bytes=None, caller_session_id=caller_session_id)
        ).decode(encoding)
    ...
    async def read_uri(
        ...
        *,
        ...
        manifest = await self._resolve_async(uri, caller_session_id=caller_session_id)
        self._assert_roots_allow(manifest, roots)
